...
"""LATEX
	...
	autocmd FileType tex inoremap ,fig \begin{figure}<Enter>\centering<Enter>\includegraphics[width=0.6\linewidth]{Bilder/<++>}<Enter>\captionsetup{format=hang}<Enter>\caption{<++>}<Enter>\label{<++>}<Enter>\end{figure}<Enter><Enter><++><Esc>/<++><Enter>NNNNc4l
